...
struct CurveMap : public std::map<std::string, Curve> {
  ...
  // define operator == for map of shared ptr: start by checking if the ptr are
  // same, otherwise check if the values are the sames
  bool operator==(const CurveMap_t& other) const {
    if (this->size() != other.size()) return false;
    for (typename Parent::const_iterator it = this->begin(); it != this->end();
         ++it) {
      if (other.count(it->first) < 1) return false;
      if ((it->second != other.at(it->first)) &&
          !(it->second->isApprox(other.at(it->first).get())))
        return false;
    }
    return true;
  }

  bool operator!=(const CurveMap_t& other) const { return !(*this == other); }
