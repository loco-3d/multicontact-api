...
struct CubicHermiteSplinePythonVisitor
  ...
  void visit(PyClass& cl) const {
    cl.def(bp::init<Index>(bp::arg("size"), "Init the hermite spline with a given size."))
        .def(bp::init<VectorX, MatrixDx, MatrixDx>(
            bp::args("abcissa", "points", "derivatives"),
            "Init from a given set of points and derivatives, together with a vector of absicca."))
        ...
        .def("numIntervals", &CubicHermiteSpline::numIntervals,
             "Returns the number of intervals contained in the spline.")
        ...
        .def("Constant", &CubicHermiteSpline::Constant, bp::arg("point"),
             "Returns a constant spline passing through the input point.")
        .staticmethod("Constant");
  }

  static void expose(const std::string& class_name) {
    std::string doc = "Cubic Hermite Spline";
    bp::class_<CubicHermiteSpline>(class_name.c_str(), doc.c_str(), bp::no_init)
 ...
 protected:
  static void setPoints(CubicHermiteSpline& self, const MatrixDx& points) { self.points() = points; }
...
void exposeSplineAlgos() {
  bp::def("createHermiteSplineAtAbsicca", &multicontact_api::trajectories::createHermiteSplineAtAbsicca<double, 3>,
