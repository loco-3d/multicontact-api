...
struct ConstraintBase {
  ...
  ReturnType value(const ArgumentType& point) const { return derived().value(point); }

  ReturnType residu(const ArgumentType& point) const { return derived().residu(point); }
};
