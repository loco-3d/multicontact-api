...
struct traits<ContactConstraintSOC<SOC> > {
  ...
  enum { dim_in = SOC::dim, dim_out = 1 };
  typedef typename SOC::VectorD ArgumentType;
  typedef Scalar ReturnType;
};

template <class _SOC>
struct ContactConstraintSOC : public ConstraintBase<ContactConstraintSOC<_SOC> >,
  ...
  ContactConstraintSOC() : m_socw(), m_factor(1.), m_threshold(0.) {}

  ContactConstraintSOC(const SOC& socw, const Scalar factor = 1., const Scalar threshold = 0.)
      : m_socw(socw), m_factor(factor), m_threshold(threshold) {}
