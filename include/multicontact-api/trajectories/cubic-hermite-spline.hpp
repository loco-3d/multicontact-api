...
template <typename _Scalar, int _dim>
struct CubicHermiteSplineTpl : public serialization::Serializable<CubicHermiteSplineTpl<_Scalar, _dim> > {
  ...
  CubicHermiteSplineTpl() : m_absicca(), m_points(_dim, 0), m_derivatives(_dim, 0), m_ds() {}

  CubicHermiteSplineTpl(const Index size)
      : m_absicca(size), m_points(_dim, size), m_derivatives(_dim, size), m_ds((size - 1 > 0) ? size - 1 : 0) {}
  ...
  CubicHermiteSplineTpl(const Eigen::MatrixBase<VectorDerived>& absicca,
      ...
      : m_absicca(absicca), m_points(points), m_derivatives(derivatives), m_ds(absicca.size() - 1) {
  ...
  CubicHermiteSplineTpl(const CubicHermiteSplineTpl<OtherScalar, dim>& other)
      : m_absicca(other.m_absicca), m_points(other.m_points), m_derivatives(other.m_derivatives) {
    compute();
  }
  ...
  bool operator==(const CubicHermiteSplineTpl<OtherScalar, dim>& other) const {
    return m_absicca == other.m_absicca && m_points == other.m_points && m_derivatives == other.m_derivatives &&
           m_ds == other.m_ds;
  }
  ...
  CubicHermiteSplineTpl operator+(const CubicHermiteSplineTpl<OtherScalar, dim>& other) const {
    ...
    return CubicHermiteSplineTpl(m_absicca, m_points + other.m_points, m_derivatives + other.m_derivatives);
  }
  ...
  void eval(const Scalar t, const Eigen::MatrixBase<Derived1>& p, const Eigen::MatrixBase<Derived2>& m) const {
 ...
 protected:
  static void evalCoeffs(const Scalar t, Scalar& h00, Scalar& h10, Scalar& h01, Scalar& h11) {
    h00 = 1.;
    h10 = t;
    h01 = 0.;
    h11 = 0.;
  ...
  ///
  bool check() const { return (m_ds.array() > 0.).all(); }
...
template <typename Scalar, int _dim>
CubicHermiteSplineTpl<Scalar, _dim> createHermiteSplineAtAbsicca(
    const CubicHermiteSplineTpl<Scalar, _dim>& spline,
    const typename CubicHermiteSplineTpl<Scalar, _dim>::VectorX& absicca) {
